...
export class APIService {
    ...
    private handleError(response) {
        ...
        if (response.ok)
            ...
            APIService.toastMessageEvent.emit({msg:"your are not authorized to do this action",title: "Unauthorized"});
        }
        else if (response.status == 404) {
            APIService.toastMessageEvent.emit({msg:"The Item you are looking for is not Found", title:"Not Found"});
        }
        else if (response.status == 500) {
            APIService.toastMessageEvent.emit({msg:"Something wrong happen.", title:"Unknown Error"});
        }
