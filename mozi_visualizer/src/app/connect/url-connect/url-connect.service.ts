...
export class UrlConnectService {
    ...
    private handleError(response) {
        ...
        if (response.ok)
            ...
            UrlConnectService.toastMessageEvent.emit({msg:"The Server is down!",title:"Network Error"})
        }
            ...
            UrlConnectService.toastMessageEvent.emit({msg:"your are not authorized to do this action",title: "Unauthorized"});
        }
        else if (response.status == 404) {
            UrlConnectService.toastMessageEvent.emit({msg:"The Item you are looking for is not Found", title:"Not Found"});
        }
        else if (response.status == 500) {
            UrlConnectService.toastMessageEvent.emit({msg:"Something wrong happen.", title:"Unknown Error"});
        }
